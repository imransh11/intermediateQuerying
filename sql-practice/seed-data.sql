...
VALUES ('The Falling Box', 1, 2015, 25000),
  ...
  -- SELECT * FROM albums WHERE band_id=1 OR band_id=3 OR band_id=4;

  SELECT * FROM albums WHERE title LIKE 'THE%' -- title start with 'the'

  -- SELECT * FROM albums order by numbers  --Not Finished

  -- SELECT BY COUNT(num_sold), num_sold From albums
  -- Gr
